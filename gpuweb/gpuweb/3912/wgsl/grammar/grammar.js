// Copyright (C) [2023] World Wide Web Consortium,
        ...
        attribute: $ => choice(
            seq(token('@'), $.ident_pattern_token, optional($.argument_expression_list)),
            seq(token('@'), token('diagnostic'), $.diagnostic_control)
        ),
        diagnostic_control: $ => seq(token('('), $.severity_control_name, token(','), $.diagnostic_rule_name, optional(token(',')), token(')')),
